...
public class HelloWorldBuilder extends Builder implements SimpleBuildStep {
    ...
    public HelloWorldBuilder(String username, String password, String ctServer, File file, Integer max_number_of_critical, Integer max_number_of_high, String weakness_is, String condition, String project_name, String workspace ) throws IOException {
        ...
        if(max_number_of_critical == null)
            this.max_number_of_critical = 0;
        else
            this.max_number_of_critical = max_number_of_critical;

        if(max_number_of_high == null)
            this.max_number_of_high = 0;
        else
    ...
    public void perform(Run<?, ?> run, FilePath workspace, EnvVars env, Launcher launcher, TaskListener listener) throws InterruptedException, IOException, AbortException {
            ...
            while(true){
                ...
                if (jsonNode.get("state").asText().equals("end")) {
                    ...
                    table += "<tr><th style='border: 1px solid black;border-collapse:collapse;'>Weakness</th><th style='border: 1px solid black;border-collapse:collapse;'>Total Issue</th><th style='border: 1px solid black;border-collapse:collapse;'>New Issue</th></tr>";

                    StringBuilder trCritical = new StringBuilder("<tr>\n");
                    trCritical.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append("🔴 Critical").append("</td>\n");
                    trCritical.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(critical).append("</td>\n");
                    trCritical.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(newIssuesSeverity.get("critical")).append("</td>\n");
                    trCritical.append("</tr>");

                    StringBuilder trHigh = new StringBuilder("<tr>\n");
                    trHigh.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append("🟠 High").append("</td>\n");
                    trHigh.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(high).append("</td>\n");
                    trHigh.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(newIssuesSeverity.get("high")).append("</td>\n");
                    trHigh.append("</tr>");

                    StringBuilder trMedium = new StringBuilder("<tr>\n");
                    trMedium.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append("🟡 Medium").append("</td>\n");
                    trMedium.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(medium).append("</td>\n");
                    trMedium.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(newIssuesSeverity.get("medium")).append("</td>\n");
                    trMedium.append("</tr>");

                    StringBuilder trLow = new StringBuilder("<tr>\n");
                    trLow.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append("🔵 Low").append("</td>\n");
                    trLow.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(low).append("</td>\n");
                    trLow.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(newIssuesSeverity.get("low")).append("</td>\n");
                    trLow.append("</tr>");
                    ...
                    trTotal.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append("🔘 TOTAL").append("</td>\n");
                    trTotal.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(total).append("</td>\n");
                    trTotal.append("  <td style='border: 1px solid black;border-collapse:collapse;'>").append(totalCountNewIssues).append("</td>\n");
                    trTotal.append("</tr>");
                ...
                if(condition == "OR"){
                        if(critical > max_number_of_critical){
                            run.addAction(new HelloWorldAction(html));
                            throw new AbortException(" ---> Critical limit exceeded");
                        }
                        if(high > max_number_of_high){
                            run.addAction(new HelloWorldAction(html));
                        ...
                        if(critical > max_number_of_critical && high > max_number_of_high && weaknessIsCount.size() > 0 ){
                            run.addAction(new HelloWorldAction(html));
